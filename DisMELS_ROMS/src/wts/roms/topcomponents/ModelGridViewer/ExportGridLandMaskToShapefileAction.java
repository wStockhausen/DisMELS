...
/**
 * This is a context sensitive action to turn on the "Export ROMS grid land mask to shapefile" menu item 
 * functionality associated with the MapViewerTopComponent mapGUI component.
 * 
 * @author William.Stockhausen
 */
public class ExportGridLandMaskToShapefileAction extends AbstractAction implements LookupListener, ContextAwareAction {
...
//    private static final Logger logger = Logger.getLogger(ExportGridToShapefileAction.class.getName());
    ...
    public ExportGridLandMaskToShapefileAction(){
        this(Utilities.actionsGlobalContext());
//        logger.info("===Creating AddGISLayerAction instance via default constructor");
    }
    
    public ExportGridLandMaskToShapefileAction(Lookup lookup){
        super("Export ROMS grid land mask to shapefile");
...
//        logger.info("actionPerformed in AddGISLayerAction");
        if ((lkpResult!=null)&&(!lkpResult.allInstances().isEmpty())){
            ...
            tc.exportGridLandMaskToShapefile();
        }
    }
...
//            logger.info("===setEnabled true");
    ...
    public Action createContextAwareInstance(Lookup actionContext) {
        return new ExportGridLandMaskToShapefileAction(actionContext);
    }
