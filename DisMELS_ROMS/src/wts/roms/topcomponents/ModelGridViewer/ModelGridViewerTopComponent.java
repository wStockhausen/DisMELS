...
//            }
  ...
  3. identifies itself as a PropertyChangeListener on the ROMS GloballInfo instance.
    ...
    /**
     * Export the model grid bathymetry as polygons to a shapefile
     */
    public void exportGridBathymetryToShapefile(){
        jfcShape.setDialogTitle("Select output shapefile for grid bathymetry");
        int res = jfcShape.showSaveDialog(this);
        if (res==JFileChooser.APPROVE_OPTION) {
            try {
                File fshp = jfcShape.getSelectedFile();                                                      
                URL url = fshp.toURI().toURL();
                FeatureCollection fc = tcMapViewer.getGrid2DMapData().getBathymetry();
                ShapefileCreator sc = new ShapefileCreator();
                sc.setShapefileURL(url);
                sc.createShapefile(fc);
            } catch (MalformedURLException ex) {
    ...
    /**
     * Export the model grid land mask as polygons to a shapefile
     */
    public void exportGridLandMaskToShapefile(){
        jfcShape.setDialogTitle("Select output shapefile for grid land mask");
        int res = jfcShape.showSaveDialog(this);
        if (res==JFileChooser.APPROVE_OPTION) {
            try {
                File fshp = jfcShape.getSelectedFile();                                                      
                URL url = fshp.toURI().toURL();
                FeatureCollection fc = tcMapViewer.getGrid2DMapData().getMask();
                ShapefileCreator sc = new ShapefileCreator();
                sc.setShapefileURL(url);
                sc.createShapefile(fc);
            } catch (MalformedURLException ex) {
